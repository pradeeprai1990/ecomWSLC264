...
let viewsubcategory = (req, res) => {
    ...
    let search = {};
    if (req.query.subcategoryName !== "" && req.query.subcategoryName !== undefined) {
      search.subcategoryName= new RegExp(req.query.subcategoryName, "i");
    }
  subcatmodal
    .find(search).populate('parentcategory')
    .then((result) => {
      let obj = {
        status: 1,
        data: result,
...
module.exports = { addsubcategory, viewsubcategory, deletesubcat, deletemulti , editcategory , updatesubcategory,parentcategory };
